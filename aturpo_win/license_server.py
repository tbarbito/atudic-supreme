...
def create_license():
    ...
    try:
        ...
        cursor.execute("""
            ...
            (license_key, customer_name, customer_email, company_name, plan_type,
             created_at, expires_at, is_active, max_users, max_repos, features)
...
def admin_create_license():
    ...
    try:
        ...
        if not customer_name or not company_name or not plan_type:
            return jsonify({
                'success': False,
        ...
        cursor.execute("""
            INSERT INTO licenses 
            (license_key, customer_name, company_name, plan_type, expires_at, 
...
def admin_block_license(license_key):
    ...
    try:
        ...
        cursor.execute("""
            ...
            SET is_active = %s
            WHERE license_key = %s
        """, (is_blocked, license_key))
        
        conn.commit()
        conn.close()
        
        status_text = 'bloqueada' if is_blocked else 'desbloqueada'
